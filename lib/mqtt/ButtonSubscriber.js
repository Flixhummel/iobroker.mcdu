...
class ButtonSubscriber {
    ...
    constructor(adapter, mqttClient, inputModeManager = null) {
        ...
        /** @type {Map<string, number>} Per-button debounce timestamps */
    ...
    async handleButtonEvent(topic, message) {
        try {
            ...
            // Per-button debounce: ignore same button pressed within window
            const now = Date.now();
            const lastPress = this.buttonDebounce.get(button) || 0;
            if (now - lastPress < this.debounceMs) {
                this.adapter.log.debug(`Button ${button} debounced`);
                return;
            }
            this.buttonDebounce.set(button, now);
    ...
    async handleFunctionKey(button) {
        ...
        // Clear input/edit mode for all function keys (except PREV/NEXT PAGE)
        if (button !== 'PREV_PAGE' && button !== 'NEXT_PAGE') {
            if (this.inputModeManager) {
                const currentMode = this.inputModeManager.getMode();
                if (currentMode === 'edit' || currentMode === 'input') {
                    this.inputModeManager.getScratchpad().clear();
                    await this.inputModeManager.getScratchpad().render();
                    await this.inputModeManager.setState('normal');
                    this.adapter.log.debug(`${button} cleared ${currentMode} mode`);
                }
            }
        }
    ...
    isKeypadButton(button) {
        if (button.length === 1 && /^[0-9A-Z]$/.test(button)) return true;
    ...
    async handlePlusMinus() {
        if (!this.inputModeManager) return;
