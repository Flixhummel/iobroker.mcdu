...
class InputModeManager {
    ...
    async handleKeyInput(char) {
        ...
        if (this.mode === 'input' || this.mode === 'edit') {
            // Refresh timeout on every keystroke (prevents timeout during active typing)
            this.modeChangeTime = Date.now();
    ...
    async handleCLR() {
        ...
        // Priority 0: Double-CLR detection (only if last CLR did something meaningful)
        ...
        // CLR did nothing meaningful — don't count for double-CLR
     ...
     */
    async checkTimeout() {
        // Timeout only applies to edit mode (field editing). Input mode (scratchpad
        // typing) has no timeout — scratchpad persists until CLR.
        if (this.mode === 'edit') {
