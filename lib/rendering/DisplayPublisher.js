...
class DisplayPublisher {
    ...
    async publishToMqtt(update) {
        ...
        const topic = topicMap[update.type];
        if (!topic) {
        ...
        try {
            ...
            this.adapter.log.error(`Failed to publish ${update.type} update: ${error.message}`);
        }
    }
