...
class PageRenderer {
    ...
    effectiveDisplayType(displayConfig) {
        if (!displayConfig) return 'empty';
        if (displayConfig.type && displayConfig.type !== 'empty') return displayConfig.type;
        if (displayConfig.text || displayConfig.label) return 'label';
        return 'empty';
    }
