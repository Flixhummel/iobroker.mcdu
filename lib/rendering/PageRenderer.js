...
const { normalizeLine, getDisplayText, migrateOldRows } = require('../utils/lineNormalizer');
...
class PageRenderer {
    ...
    async renderPage(pageId) {
        try {
            ...
            // Normalize all lines to new format, migrating old row numbers if needed
            const normalizedLines = migrateOldRows(pageConfig.lines.map(l => normalizeLine(l)));
    ...
    renderStatusBar(pageId) {
        ...
        if (breadcrumbText.length > maxNameLen && breadcrumb.length > 2) {
            ...
            const shortened = breadcrumb.map((b, i) => {
                if (i === 0 || i === breadcrumb.length - 1) return this.sanitizeAscii(b.name.toUpperCase());
