...
class PageRenderer {
    ...
    constructor(adapter, displayPublisher, scratchpadManager = null) {
        ...
        /** @type {number} Half-width for left/right columns */
        ...
        /** @type {number} Current page offset for pagination */
        this.currentPageOffset = 0;

        /** @type {number} Total pages for pagination */
        this.totalPages = 1;
    }
    ...
    async renderPage(pageId) {
        try {
            ...
            for (let row = 1; row <= this.rows; row++) {
                try {
                    // Even rows (2,4,6,8,10): render sub-labels for the NEXT odd row
                    if (row % 2 === 0 && row <= 10) {
                        const nextOddRow = row + 1;
                        const nextLineConfig = this.totalPages > 1
                    ...
                    const lineConfig = this.totalPages > 1 && oddRows.includes(row)
            ...
            // List layout: add scroll indicators
            const layoutType = pageConfig.layoutType || 'menu';
            if (layoutType === 'list' && this.totalPages > 1) {
    ...
    lineHasDisplay(line) {
        ...
        const leftHas = line.left?.display?.type && line.left.display.type !== 'empty';
        const rightHas = line.right?.display?.type && line.right.display.type !== 'empty';
    ...
    renderSubLabel(lineConfig) {
        ...
        if (leftLabel && rightLabel) {
            ...
            const leftPart = ` ${leftLabel}`;
            const rightPart = `${rightLabel} `;
            const gap = this.columns - leftPart.length - rightPart.length;
            text = leftPart + ' '.repeat(Math.max(0, gap)) + rightPart;
        } else if (leftLabel) {
            text = ` ${leftLabel}`;
        } else if (rightLabel) {
            text = rightLabel;
            text = ' '.repeat(this.columns - text.length - 1) + text + ' ';
        } else {
            text = '';
        }
    ...
    renderStatusBar(pageId) {
        ...
        if (breadcrumb.length > 1) {
            ...
            breadcrumbText = breadcrumb.map(b => b.name.toUpperCase()).join(' > ');
        } else {
            // Single page (root) - just show page name
            const pageConfig = this.findPageConfig(pageId);
            breadcrumbText = (pageConfig?.name || pageId).toUpperCase();
        }
        ...
        if (breadcrumbText.length > maxNameLen && breadcrumb.length > 2) {
            ...
            const shortened = breadcrumb.map((b, i) => {
                if (i === 0 || i === breadcrumb.length - 1) return b.name.toUpperCase();
        ...
        const truncatedName = breadcrumbText.length > maxNameLen
            ? breadcrumbText.substring(0, maxNameLen)
            : breadcrumbText;
    ...
    async renderLine(pageId, lineConfig, row) {
        ...
        if (leftHasContent && rightHasContent) {
            ...
            color = leftResult.color; // left side determines line color
        ...
        // Menu layout: add < > indicators for selectable items
        const pageConfig = this.findPageConfig(pageId);
        const layoutType = pageConfig?.layoutType || 'menu';
        if (layoutType === 'menu' && normalized) {
            const leftHasButton = normalized.left?.button?.type && normalized.left.button.type !== 'empty';
            const rightHasButton = normalized.right?.button?.type && normalized.right.button.type !== 'empty';

            if (leftHasButton || rightHasButton) {
                const trimmed = text.trim();
                if (trimmed.length > 0) {
                    // Fit text inside < > without cutting characters
                    const inner = this.padOrTruncate(trimmed, this.columns - 2);
                    text = '<' + inner + '>';
                }
            }
        }
    ...
    async renderSideDisplay(displayConfig, row) {
        if (!displayConfig || displayConfig.type === 'empty') {
        ...
        if (displayConfig.type === 'label') {
            text = getDisplayText(displayConfig);
        } else if (displayConfig.type === 'datapoint') {
    ...
    async renderDatapoint(displayConfig, row) {
        const { source, format, unit, editable } = displayConfig;
    ...
    addEditIndicator(content, isEditable, isActive) {
        ...
        if (isActive) {
            return { text: `[${content}]`, color: 'amber' };
        }
        return { text: `${content} <-`, color: 'amber' };
    }
