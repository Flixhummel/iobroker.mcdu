 ...
 *
 * Nested format:
 *   { row, left: { label, display: { type, text, color }, button: { type, target } }, right: {...} }
 *
 * Flat format (Admin UI):
 *   { slot, leftLabel, leftText, leftColor, leftButtonType, leftTarget, right... }
 */
...
function unflattenLine(flat) {
    ...
    const leftDisplayType = (flat.leftButtonType === 'datapoint' && flat.leftSource)
        ? 'datapoint'
        : (flat.leftText ? 'label' : 'empty');
    const rightDisplayType = (flat.rightButtonType === 'datapoint' && flat.rightSource)
        ? 'datapoint'
