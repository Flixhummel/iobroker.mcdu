...
function flattenLine(line) {
    ...
    return {
        ...
        leftColData: n.left?.display?.colData || '',
        ...
        rightColData: n.right?.display?.colData || '',
...
function unflattenLine(flat) {
    ...
    const row = flat.slot !== undefined && flat.slot !== null ? slotToDataRow(Number(flat.slot)) : 3;

    const leftDisplayType = flat.leftButtonType === 'datapoint' ? 'datapoint' : flat.leftText ? 'label' : 'empty';
    const rightDisplayType = flat.rightButtonType === 'datapoint' ? 'datapoint' : flat.rightText ? 'label' : 'empty';
